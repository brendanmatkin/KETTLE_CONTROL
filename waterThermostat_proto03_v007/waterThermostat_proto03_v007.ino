...
OneWire oneWire(thermoPin);        // thermometer pin
DallasTemperature thermos(&oneWire);
...
void setup() {
  ...
  // thermometer init
  thermos.begin();
  thermos.getAddress(thermometer, 0);     // location index 0 on bus
  thermos.setResolution(thermometer, tempRes);  // 9-12 bit
  thermos.setWaitForConversion(false);
